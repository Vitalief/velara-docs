...
> **⚠️ FR-ID note (2026-07-24).** The per-epic **"FRs covered"** lines below use a **legacy `FR-<DOMAIN>-NN` prefix** inherited from the now-retired `requirements-inventory.md`. The **authoritative Functional Requirements registry is the PRD** (`prds/prd-Velara-2026-05-29/prd/5-functional-requirements.md`), which uses **bare IDs** (`USE-09`, not `FR-USE-09`). Where the two ever disagree, **the PRD wins** — e.g. this index historically used `FR-USE-07`/`FR-USE-08` for cost/run-history, but the PRD's real `USE-07`/`USE-08` are audit-name-resolution / audit-icons; cost is **USE-09**, LLM observability **USE-10**, cert-evidence gate **CRT-06**. The legacy prefixes on **done-epic** rows are left as historical record; the active Epics 15/16/17 lines have been reconciled to PRD IDs.
>
...
>
> **Added 2026-07-20** (see `sprint-change-proposal-2026-07-20-cost-tracking.md`). **New Epic 15: Per-Execution Cost Tracking** — an operator found only a single platform-wide aggregate dollar figure (Analytics Overview's `token_cost`); no per-invocation, per-skill, or per-user cost anywhere, and the AI-adapter-propose LLM spend (Epic 11/14) is captured in tokens but never priced either. Adds **USE-09** (PRD §5.11; formerly mis-cited as `FR-USE-07`). **Epic 9 stays `done`** (USE-06 fully met as originally scoped) — this is new scope, not a defect.
>
> **Added 2026-07-20** (see `sprint-change-proposal-2026-07-20-engagement-model-refinement.md`). **New Epic 16: Engagement Hierarchy, Attachment & Ingest Model Refinement** — five real usage-friction points: Locations re-created per Study (moving to Client-owned + Study-associated), skill attachment stuck at Project/Study (adding a Client level), documents re-uploaded at every invocation (moving the protocol to Study-creation time), cluttered engagement-screen action buttons (consolidating into one menu), and no hierarchy-level visibility into run outputs (new Project/Study "Recent Runs" panel). Supersedes ORG-03; clarifies REG-04; extends INV-09 and **USE-03** (hierarchy-scoped run history); adds REG-10/ING-05 (PRD §5; formerly mis-cited with an `FR-` prefix, and run-history was mis-numbered `FR-USE-08` — real USE-08 is audit icons). **Epics 3, 4, 5, and 8 all stay `done`** — this is forward-amending scope, not a defect in any of them. The Location data migration (Story 16.1) is real-data, not additive-nullable, and is isolated as its own first story with an architect review recommended before implementation.
>
> **Added 2026-07-24** (see `sprint-change-proposal-2026-07-24.md`). **Epic 16 amended** with two FE-only deployed-dev fixes — 16.7 (Run Console reopens a stale completed job) and 16.8 (engagement "Run" shows the whole skill picker instead of locking to the clicked skill). **New Epic 17: LLM-Call Observability & Certification Evidence** — LangSmith per-individual-LLM-call tracing at the platform LLM seam (17.1) plus an AI-adapter re-change so authored/upgraded skill bundles emit traced in-sandbox calls (17.2, à la Epic 14.2), and a certification 5-run dry-run evidence gate before the technical key can be turned (17.3). Adds **USE-10** (LLM observability) and **CRT-06** (cert evidence gate) to PRD §5 (formerly provisional `FR-USE-09`/`FR-CERT-04`). **Epics 6 and 15 stay `done`** — forward-amending scope. LangSmith needs an architecture ADR (external dep + IP/PII-in-traces boundary) before 17.1. A third request (version-selector role gate) was **dropped as already satisfied by Story 11.7**.
...
> **Amended 2026-07-24** via correct-course (`sprint-change-proposal-2026-07-24.md`): two FE-only deployed-dev fixes appended.
> - 16.7 Run Console no longer reopens a stale completed job _(FE bug — narrow the sessionStorage restore; in-flight jobs must still survive a refresh)_
> - 16.8 Engagement "Run" opens the console locked to that one skill _(FE fix — reuse skill-first mode's locked card; picker survives only for a no-skill context launch)_

---

## Epic 17: LLM-Call Observability & Certification Evidence

<!-- Added 2026-07-24 via correct-course. Authoritative story-able version: epics/epic-17-observability-and-certification-evidence.md. -->

Two operator asks. First, cost/observability at the level of each **individual LLM call** — Epic 15 stores a per-execution cost, but there is no per-call trace, and no visibility at all into the LLM calls a code-driven hybrid skill makes inside its own sandbox. This epic wires LangSmith at the single platform LLM seam (`LLMProvider`) so every platform call emits a traced, costed span (17.1), then re-changes the AI adapter so newly authored/upgraded **skill bundles** emit LangSmith-traced in-sandbox calls too (17.2) — the same authoring-seam Epic 14.2 changed. Second, a governance-evidence gap: an ma_tech member can turn the technical certification key (Epic 6.2) with no recorded proof the skill was exercised. This epic adds a **5-run dry-run evidence trail** that must exist for a skill version before its technical key can be recorded (17.3), reusing the real invocation path and preserving 6.1's append-only immutability and the FR-SEC-10 signature manifestation.

**Epics 6 and 15 stay `done`** — this epic extends their scope forward (LangSmith is observability atop 15's stored cost; the cert dry-run gate is a new pre-condition on 6.2's technical key), the same pattern Epic 15 used on Epic 9 and Epic 16 used on 3/4/5/8.

**A LangSmith architecture ADR** (external dependency, config/secret management, IP/PII-in-traces boundary) should land before 17.1.

**Stories (to be detailed via create-story):**
- 17.1 LangSmith tracing for platform LLM calls _(needs the LangSmith ADR first; config-gated safe-by-default; additive to Epic 15 stored cost)_
- 17.2 AI adapter emits LangSmith-traced skill bundles _(depends on 17.1; mutates authored bundles — higher risk, isolate per the 14.2 precedent; existing-bundle re-adapt-vs-forward is an explicit in-story decision)_
- 17.3 Certification dry-run evidence gate — 5 runs before the technical key _(independent; new `CERTIFICATION_EVIDENCE_INSUFFICIENT` 422 gate; technical-only; fresh trail per new version; preserves 6.1 immutability + FR-SEC-10)_

**FRs covered:** **USE-10** _(new — call-level LLM observability via LangSmith)_, **CRT-06** _(new — technical-certification evidence gate)_ — reconciled 2026-07-24 to PRD §5 (were provisional `FR-USE-09`/`FR-CERT-04`; USE-09 was taken by Epic 15's cost row).

**Dropped (not an epic story):** the "version selector admin/ma_tech only" request — already satisfied end-to-end by Story 11.7 (`invocations.py:_resolve_target_version` ignores a non-grantor's version pin → runs `current_version`; FE `VersionSelector` gated behind `isGrantor()`).
