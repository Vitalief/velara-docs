...
> **Amended 2026-07-20** (`sprint-change-proposal-2026-07-20.md`): skill-upgrade flexibility (new **Epic 14**). **SKL-02** — a hybrid skill's manifest shape (LLM-driven ↔ code-driven) is **no longer locked across versions**; a shape change is a normal new version (Epic 14 Story 14-1). This supersedes the Story 5.5.1 "single manifest shape per skill" invariant: the skill-row `schema_version`/`egress` become a re-derived projection of the current version, egress snapshotted per-version. **SKL-03** — the AI adapter's propose→approve loop **extends to the create-version (upgrade) path**, not just initial registration (Epic 14 Story 14-2); the byte-for-byte-core / checksum-proven / two-key-recert guarantees are unchanged. **REG-02** — the **explicit version number is now selectable in the UI** on new-version authoring (Epic 14 Story 14-3): omitting it preserves the auto-minor-bump; an explicit value must be strictly-greater semver (`INVALID_VERSION` otherwise). Backend already supported the optional `version`; this closes the FE gap.
