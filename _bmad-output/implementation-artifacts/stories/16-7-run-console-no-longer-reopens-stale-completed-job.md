...
_Code review 2026-07-27 (bmad-code-review, 3-layer adversarial: Blind Hunter + Edge Case Hunter + Acceptance Auditor). All 4 ACs verified satisfied for shipped behavior; `tsc --noEmit` clean, `vitest run` 139/139 green on the 3 touched test files. 0 decision-needed, 2 patch, 2 defer, 6 dismissed as noise. Severity is by production consequence — StrictMode double-invoke does NOT run in the production build, which is where the fixed bug lives._

- [x] [Review][Patch] StrictMode dev-race can null `explicitJobId` before the guard reads it, clearing an explicitly-opened terminal job [velara-web/src/features/run/components/RunConsole.tsx:1191] — FIXED. `<StrictMode>` (main.tsx:41) runs each effect mount→cleanup→mount in dev. On an explicit open of an *already-terminal* job whose data is fetched cold (`useJob` `staleTime:0`, so `job` is `undefined` on the first pass), the throwaway-mount cleanup `() => clearExplicitJobId()` sets `explicitJobId=null` before the guard re-runs on the resolved terminal job; the guard then hit the `job.id !== hydratedJobId` branch and called `setActiveJobId(null)`, wrongly clearing the explicitly-opened job. **Production build (StrictMode off) was unaffected** — dev-only fragility. Fix: added `explicitlyOpenedIdRef` (a `useRef` that latches the job id whenever the live store `explicitJobId` matches the current `activeJobId`); the guard now treats a job as explicit if `job.id === explicitJobId` OR `job.id === explicitlyOpenedIdRef.current`. Refs survive StrictMode's synthetic remount (only the store field is nulled by the cleanup), so the latched explicit intent persists; a genuine unmount discards the whole instance (ref included), so a later separate mount starts with an empty latch and a stale leftover is still correctly cleared. `tsc`/`eslint` clean, `vitest run` 791/791 green. (No inline regression test added — see defer item below: the mocked-store harness resolves `useJob` synchronously and cannot reproduce the async-after-cleanup timing, so a mock-based StrictMode test passes even with the fix reverted; a meaningful test needs the real-store harness, deferred.)
- [x] [Review][Patch] New docstrings assert a "Jobs pick → navigate to Run Console (setActiveJobId before nav)" flow that does not exist in the code [velara-web/src/stores/useRunStore.ts:57] — FIXED. The comments at `useRunStore.ts:56-58`/`19-23` and `RunConsole.tsx:1156-1160` stated the Jobs-history pick sets `activeJobId` "before the navigation to Run Console". In actual source, `JobsHistory.handleSelect` (JobsHistory.tsx:265) renders the job **inline** in its own `JobDetailPanel` and never navigates; the only Jobs→console path, "Open in Run Console →" (JobsHistory.tsx:184), navigates URL-only and does **not** call `setActiveJobId`. Rewrote the three docstrings to describe the paths that actually set `explicitJobId` — in-console submit and "view prior" on a duplicate check, both firing while `JobStatusPanel` is already mounted in `RunShell` — and dropped the non-existent Jobs-screen-navigation claim.
- [x] [Review][Defer] One-frame flash of the stale terminal panel before the guard clears it [velara-web/src/features/run/components/RunConsole.tsx:1170] — deferred, pre-existing. The guard is a `useEffect` (runs after paint), so a stale terminal `activeJobId` can render its full panel for one frame before `setActiveJobId(null)`. The pre-change hydration guard (HEAD) had the identical fetch→render→clear ordering for the refresh case — this change extends the same mechanism, it does not introduce the flash. Loading skeleton usually masks it. A synchronous known-terminal clear (or gating the render on the guard state) would remove it entirely; not blocking.
- [x] [Review][Defer] Component tests mock `useRunStore` wholesale, so the StrictMode / unmount-cleanup lifecycle is never exercised [velara-web/src/features/run/components/RunConsole.test.tsx:54] — deferred, pre-existing test-layer split (Trap 3). Store semantics ARE covered in `useRunStore.test.ts`, but no test renders `JobStatusPanel` against the real store through a mount→unmount→remount (or StrictMode) cycle — which is exactly why the dev-race patch finding above slips CI. Closing this gap (a real-store integration test driving the panel lifecycle) is follow-up work, not a blocker for this story.
