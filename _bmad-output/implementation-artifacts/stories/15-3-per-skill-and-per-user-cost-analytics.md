...
### Review Findings — 2026-07-22 (independent re-review)

Second independent 3-layer code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor, all at Opus capability). Acceptance Auditor re-confirmed all 5 ACs MET, File List an exact match, every out-of-scope constraint honored, and both emphasized invariants (no fan-out cost double-count via parent `cost_usd=0`; COALESCE on every SUM) verified in source. 1 patch, 3 deferred (all pre-existing/latent), 4 dismissed as noise.

- [x] [Review][Patch] `fmtUsd` lacks the null/NaN guard its sibling formatters have [velara-web/src/features/analytics/analyticsFormat.ts:14] — FIXED 2026-07-22 (widened signature to `number | null | undefined`, returns `"—"` for null/NaN; tsc/eslint/vitest all green) — `fmtMs`/`fmtTrend` both return `"—"` for `null`/`undefined`; `fmtUsd` calls `Intl.format(n)` directly, so `fmtUsd(undefined)` → `"$NaN"` and `fmtUsd(null)` → `"$0.00"`. This story's `cost_usd` contract is non-null so it cannot fire today, but a stale/legacy/partial payload (or any future optional cost field routed through `fmtUsd`) would render `$NaN` in the UI. Cheap defensive fix; matches AC5/Task 8's own stated rationale for wanting a null-safe wrapper.
- [x] [Review][Defer] `_invocation_where` comment falsely claims the fan-out PARENT row has `outcome=NULL` [velara-api/app/services/analytics_service.py:66-71] — deferred, pre-existing. `execution_tasks.py:667,681` writes the parent audit row with `outcome="success"`/`"failure"` (non-NULL), so the parent is NOT excluded by `AuditLogEntry.outcome.isnot(None)` — it IS counted in `runs`/`invocations`/every aggregate, and this story's cost sum joins over it too. Cost double-counting is avoided ONLY because `execution_tasks.py:674-680` writes the parent's `InvocationResult.cost_usd=0` (terminal-guarded to exactly one parent row). Holds today; but the comment actively misleads a future editor into believing the parent is filtered out — if anyone later prices the parent roll-up non-zero, every fan-out job's cost double-counts silently. The matching test (`test_overview_excludes_admin_and_fan_out_parent_rows`, `test_analytics.py:398-404`) seeds `outcome=None` — a shape production never emits — giving false confidence. Same item the first review deferred; independently re-confirmed in source.
- [x] [Review][Defer] New `_token_cost` silently broadens `token_cost` to include `blocked`-run cost the old JSONB path excluded, with no test pinning the delta [velara-api/app/services/analytics_service.py:173-190] — deferred, low-impact. Old code filtered `event_metadata.isnot(None)` (only prompt/hybrid rows contributed); the new column-sum includes any invocation-leaf row with a matching priced `InvocationResult`, which now includes `blocked` runs (`job_service.mark_blocked` writes a result with a real `cost_usd`). Plausibly the intended, more-correct behavior (AC3 explicitly wants failed/code-runtime rows correctly included), but it is an unremarked value change to the platform-wide figure and no test asserts a `blocked` row's cost lands in the overview total.
- [x] [Review][Defer] Non-DISTINCT `InvocationResult` join could double-count cost if two terminal audit rows ever shared one `job_id` [velara-api/app/services/analytics_service.py:131-154,173-190,254-283] — deferred, pre-existing/latent. Celery autoretry is deliberately disabled (`execution_tasks.py:18-24`), and the identical join already backs `func.count()` (`runs`) in the same queries, so any second-terminal-row-per-`job_id` duplication is a pre-existing audit-row-model property `runs` already shares; cost merely inherits it. Not introduced by 15.3.

**Dismissed as noise (4):** (1) `AnalyticsUserSummary.cost_usd` added to schema/query but not rendered in the user-selector list — BY DESIGN: AC5 specifies rendering only in the detail header tile + `SkillsUsed`; the summary field satisfies AC2's schema/query contract. (2) `float(SUM(Numeric(12,6)))` sub-cent precision drift on large totals — the `token_cost: float` wire convention predates this story; float USD is consistent across the whole analytics surface. (3) `test_top_skills_cost_usd_sums_per_skill` expects 4.0 for both skills so a cross-skill attribution bug wouldn't be caught by cost alone — minor coverage nit; `runs` (2 vs 3) still distinguishes the grouping, and the test correctly exercises NULL-cost + no-result-row cases. (4) Module docstring / `_seed_invocation_result` inert `org_id`/`hierarchy_path` params — comment/test drift, no consumer impact.
