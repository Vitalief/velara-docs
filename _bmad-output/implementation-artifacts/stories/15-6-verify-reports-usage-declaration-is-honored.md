...
- [ ] **Task 1 — Static return-shape analysis (AC1, AC3) — new `app/services/usage_contract.py`**
  - [ ] `detect_reports_usage_without_emission(files, entrypoint) -> Literal["usage_detected", "usage_absent", "undetermined"]`. Reuse `entrypoint_contract.resolve_entrypoint_module` for bundle/package-root resolution; do not duplicate it.
  - [ ] Walk the resolved entrypoint's `FunctionDef` body: for each `return` statement, inspect the returned expression. A returned `ast.Dict` literal is checked directly for a string key `"usage"`. A returned `ast.Name` is traced to its last top-level assignment WITHIN the same function body (best-effort, single-hop) before falling back to `undetermined`. Anything else (comprehension, call result, conditional expression, multiple divergent return shapes) → `undetermined`.
  - [ ] No `usage` key found in ANY resolvable return path (and at least one return was resolvable) → `usage_absent`. A `usage` key found in every resolvable return path → `usage_detected`. Mixed/partial/no resolvable returns → `undetermined`.

- [ ] **Task 2 — Wire into registration gate (AC2, AC5) — `app/services/skill_service.py`**
  - [ ] In `_process_bundle` (or the same seam 15.5's `_manifest_missing_synthesizable_field` call sits in), after a manifest parses with `reports_usage: true` AND a resolvable entrypoint, call `detect_reports_usage_without_emission`. On `usage_absent`, raise the SAME `MalformedBundleManifestError` 15.5 raises for an omitted declaration (do not introduce a new error code).
  - [ ] `usage_detected` and `undetermined` both register normally (AC3 — fail open). `reports_usage: false` skips the call entirely.
  - [ ] Inline authoring path (`_require_inline_reports_usage_declaration`'s call sites): on `usage_absent`, raise the same `InvalidCodeDrivenManifestError` 15.5 raises for an undeclared inline manifest (hard 422 — no assist route available inline).

- [ ] **Task 3 — Tests (AC4, AC5)**
  - [ ] Fixture entrypoint matching the ACTUAL v2.1.0 broken shape (wraps a client call, returns its result dict unchanged, no `usage` key) → `usage_absent`.
  - [ ] Fixture entrypoint matching the hand-patched v2.2.0 shape (constructs `result["usage"] = {...}` before returning `result`) → `usage_detected` (exercises the single-hop `ast.Name`-to-assignment trace).
  - [ ] Fixture with a returned dict literal built inline including `"usage": {...}` directly → `usage_detected`.
  - [ ] Fixture with a comprehension/conditional/helper-function-derived return → `undetermined`, and confirm registration proceeds (fail-open, AC3).
  - [ ] Registration integration test: `reports_usage: true` + `usage_absent` fixture bundle → `MALFORMED_BUNDLE_MANIFEST` (bundle path) / `INVALID_CODE_DRIVEN_MANIFEST` (inline path).
  - [ ] Regression: existing 15.5 tests for `reports_usage: true`/`false` registration continue to pass unmodified.

- [ ] **Task 4 — Gates**
  - [ ] Rebuild api + worker before pytest (source is baked, not mounted).
  - [ ] Full suite green; ruff clean; no api-spec diff (internal registration logic only).
