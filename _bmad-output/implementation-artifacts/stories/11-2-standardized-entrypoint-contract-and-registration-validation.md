...
**Code review 2026-07-08 (bmad-code-review, 3-layer adversarial: Blind Hunter + Edge Case Hunter + Acceptance Auditor).** Diff reviewed: velara-api uncommitted working tree vs `007e50d` (the story's baseline_commit). **Outcome:** 11 patches ALL APPLIED same day, 1 deferred (case-insensitive-FS clobber → deferred-work.md), 8 dismissed, 0 decisions. Post-patch gates: ruff clean, 1190 passed in-container (+12 new tests; only the 3 known test_ingest MinIO failures), api-spec.json regen zero-diff.

- [x] [Review][Patch] HIGH Validator binds to the FIRST top-level definition; Python import binds to the LAST — a conforming `def run` followed by `run = SomeClass()` (or a second def) passes registration and fails every run; a non-conforming stub before a conforming def is falsely 422'd. Scan all of `tree.body` and validate the LAST binding of the callable name, matching import semantics. [velara-api app/services/entrypoint_contract.py:156-186]
- [x] [Review][Patch] HIGH `ast.parse` on untrusted source can raise `ValueError` (NUL byte — decodes as clean UTF-8), `RecursionError` (deep nesting), not just `SyntaxError` — escapes as a 500 on POST /skills instead of 422 ENTRYPOINT_CONTRACT_VIOLATION. Catch (SyntaxError, ValueError, RecursionError) → "could not be parsed". [velara-api app/services/entrypoint_contract.py:144-151]
- [x] [Review][Patch] HIGH Missing (deleted/drifted) S3 bundle member or manifest at run time → raw botocore ClientError → generic unmapped job failure — the exact deferred-work.md:388 failure class this story exists to kill; only sha-mismatch gets the clean CODE_DRIVEN_EXECUTION_ERROR. Also malformed artifact_set rows (missing "path"/"sha256" → KeyError) and materialization OSErrors bypass the mapping. Wrap the manifest fetch, member fetches, row-field access, and dest writes → CodeDrivenExecutionError. [velara-api app/services/execution_service.py:717-719; app/services/code_driven_executor.py:227-247]
- [x] [Review][Patch] HIGH Task 4's `_run_hybrid` is_bundle branch has ZERO test coverage and the "deferred-work.md:388 closure" E2E test never exercises it — it calls `run_code_driven_hybrid` directly with hand-built stubs (bypassing the manifest-member fetch, dispatch, and tampered-row branch), requests `client`/`celery_eager` fixtures it never uses, and its hand-built artifact_set omits manifest.json (would fail if actually routed through `_run_hybrid`). Add `_run_hybrid`-level bundle tests (mocked-storage pattern from test_execution_service.py) + fix the E2E test's artifact_set/fixtures. (The 11.1-review "checkbox checked but unimplemented" class, third occurrence.) [velara-api tests/integration/api/test_code_driven_execution.py:718-799; tests/unit/services/test_execution_service.py]
- [x] [Review][Patch] MEDIUM `pkg/mod.py` + `pkg/mod/__init__.py` coexisting: the resolver probes `mod.py` first, but Python's import system prefers the package directory — a conforming `mod.py` beside a non-conforming `mod/__init__.py` passes registration and fails at run time (symmetrically, so never caught). Probe `__init__.py` before `.py` to match import precedence. [velara-api app/services/entrypoint_contract.py:112]
- [x] [Review][Patch] MEDIUM File-vs-directory member collision (`a` + `a/b.py`, legal in ZIP) passes registration (no filesystem touched) then fails every run with an unmapped NotADirectoryError at materialization. Reject path-prefix conflicts at extraction (INVALID_BUNDLE). [velara-api app/services/bundle_extractor.py extract_bundle; code_driven_executor.py:245-247]
- [x] [Review][Patch] LOW UTF-8-BOM module source falsely rejected — `decode("utf-8")` preserves U+FEFF and `ast.parse` raises SyntaxError on it, 422-ing a module Python imports fine. Decode `utf-8-sig`. [velara-api app/services/entrypoint_contract.py:144-146]
- [x] [Review][Patch] LOW `_describe_bind_failure` substring-scan misattributes (a param named `params_mode` in the TypeError text blames `params`) and goes generic for the AC4-enumerated extra-required-param case (never names `model`); the paired test was weakened to assert only the code. Extract the quoted param name from the TypeError message properly; strengthen the test. [velara-api app/services/entrypoint_contract.py:243-257; tests/unit/services/test_entrypoint_contract.py]
- [x] [Review][Patch] LOW Materialization log omits `duration_ms`, contradicting the checked Task 5 subtask (the 11.1 precedent in the same file includes it). [velara-api app/services/code_driven_executor.py:261-266]
- [x] [Review][Patch] LOW No negative entrypoint-contract test on the bundle create_version branch (AC1 covers "create or new version"; only create is tested for the 422). Add one. [velara-api tests/integration/api/test_skills.py]
- [x] [Review][Patch] LOW Decorator static-analysis limitation is accepted per spec but the spec-mandated module-docstring note is missing ("a decorator can rewrap invisibly — note it in the module docstring"). Add the note. [velara-api app/services/entrypoint_contract.py docstring]
- [x] [Review][Defer] Case-colliding member paths (`pkg/Mod.py` + `pkg/mod.py`) silently clobber on case-insensitive filesystems (macOS/Windows dev machines; prod Linux unaffected) — last-written member wins undetected. — deferred, dev-environment-only
- Dismissed (8): conditional/try-block defs + lambda/re-export rejection (spec-sanctioned first cut, tested); no size/count re-check at materialization (sha256 verification subsumes size; caps enforced at registration); no post-join containment check (`_is_path_safe` verified to reject absolute/non-canonical paths); sys.path[0] stdlib shadowing (inherent to executing user code in its own venv subprocess); ERROR_CODE constant "dead weight" in worker (spec-mandated cataloguing, mirrors INVALID_BUNDLE); half-provided bundle kwargs (internal seam, single caller); private `_is_path_safe` import (spec-mandated reuse); bind-mode keyword-call assumption (verified against `_RUNNER_SCRIPT`).
