...
> **⚠️ 8.6 HANDOFF (added 2026-07-01 via create-story 8-6, sequenced BEFORE this story):** Story 8.6 lands the **real `skill_attachment` model** and OWNS the attachment-backed **`GET /api/v1/client/skills` + `ClientSkillRead`** (product decision: 8.6 builds the attachment-aware client-availability query — `attached ∩ granted ∩ client_facing/client_ready`). **If 8.6 has merged, do NOT rebuild the scope-heuristic below** (Task 1.2/1.4/1.5): the endpoint + schema already exist. 8.4's job becomes the **discovery SCREENS + project/study bucketing** (Tasks 7–10) consuming 8.6's endpoint, plus the invocation-widening D1 (Task 5, unrelated to 8.6). Keep Task 1 only as a fallback if 8.4 is picked up before 8.6 merges; otherwise treat Task 1 as "verify the 8.6 endpoint returns what the FE needs (id/name/description/scope/location_dependent) and extend it for bucketing params if missing." [Source: stories/8-6…md Task 5 + Task 11; architecture ADR #skill-attachment-model]

- [ ] **Task 1 — `GET /api/v1/client/skills` (AC1, AC2, AC5)** *(see 8.6 handoff above — likely already built by 8.6)*
...
- **8.6 LANDED (2026-07-01):** Story 8.6 has been implemented and delivered the real attachment-backed `GET /api/v1/client/skills` + `ClientSkillRead` schema (id/name/description/scope/location_dependent). Availability = `attached ∩ granted (8.1) ∩ visibility==client_facing ∩ lifecycle_state==client_ready`. **DO NOT** rebuild the scope-heuristic in Task 1.2/1.4/1.5 — the endpoint and schema exist in `velara-api/app/api/v1/client.py` and `velara-api/app/schemas/client.py`. 8.4's job is the discovery SCREENS + project/study bucketing on top of the 8.6 endpoint, plus invocation-widening D1 (Task 5). The `useProjectSkills` seam has been rewired to real attachments (Story 8.6 Task 9); the internal `EngagementsScreen` StudyDetail now shows both project-level and study-specific skills sections. The Access Control assignment UI (internal, admin-only) is also live in `features/admin/components/AccessControl.tsx`.
