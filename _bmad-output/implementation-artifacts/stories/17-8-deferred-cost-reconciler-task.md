...
_Code review 2026-07-30 (3 parallel adversarial layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor). Severities set by the reviewer after reading live source + empirical runtime checks (PG `Numeric(12,6)` NaN/Inf acceptance; langsmith 0.10.10 `Run.total_cost = Optional[Decimal]`; `list_runs` trace visibility; `parse_document` retry-scope precedent). Findings that survived verification:_

- [x] [Review][Patch] **Partial-trace visibility can write a permanent undercounted cost (AD-4 / AD-8 completeness) [app/workers/cost_reconciler.py:94-124].** _(Resolved from a decision-needed finding 2026-07-30: **Option 1 — gate on the trace's root run being complete** before trusting the child llm set. FIXED + tested.)_ `_sum_total_cost` gates on "every *currently-visible* llm run has a non-NULL `total_cost`" — it never verifies it has seen **all** of the trace's llm runs. `list_runs` is backed by an eventually-consistent index, so a multi-LLM invocation (e.g. a sonnet locator + an opus extraction) can, at T+60s, return only run A (already priced) while run B is not yet queryable. The gate passes, the CAS writes `cost_usd = cost(A)` authoritatively (`cost_is_estimated=false`), and the candidate re-check (`WHERE cost_is_estimated = true`) guarantees the row is never revisited — run B's cost is lost forever, and the discarded estimate may have been closer to truth. **Fix (Option 1):** before summing the child llm runs, require the trace's **root `chain` run** (`velara.invocation`, the last run to finalize) to be present AND complete (`end_time`/`status` set) — read it via `list_runs(trace_id=…, run_type="chain", ...)` (root = `parent_run_id is None`); if absent or not finalized, raise `_CostNotYetComputed` → retry. Only when the root is complete are all child llm runs guaranteed emitted, making the existing "every visible run is priced" check trustworthy. This restores AD-4's "sum across **all** llm runs" guarantee. Add a test: a trace whose root run is still running (or absent) → retry, no write.

- [x] [Review][Patch] **AC3's 0-row CAS "redelivery no-op" branch is untested [tests/integration/workers/test_cost_reconciler.py:329-357].** _(FIXED — added `test_second_run_hits_cas_zero_row_branch_when_row_flips_mid_flight`: races a concurrent flip-to-`false` in between the candidate re-check and the CAS UPDATE so the UPDATE matches 0 rows → `already_reconciled`, value never regressed.)_ AC3(b) mandates a test proving a redelivered run over an already-`false` row updates **0 rows** via the compare-and-set token. The current `test_redelivered_run_over_reconciled_row_is_a_zero_row_noop` seeds `cost_is_estimated=False`, so it exits at the earlier **candidate re-check** (`status == "not_a_candidate"`, `list_runs` not called) and never reaches the CAS UPDATE. The `already_reconciled`/`rowcount == 0` branch (`cost_reconciler.py:215-220`) — the actual race protection between two concurrent deliveries while the row is still `true` — has zero coverage (grep confirms no test references `already_reconciled`/`rowcount`). Add an integration test that reconciles a `true` row twice with a mocked `list_runs` and asserts the second run returns `already_reconciled` (0 rowcount, `cost_usd` unchanged). Behavior is correct; the mandated assertion is missing.

- [x] [Review][Patch] **Non-finite `total_cost` (NaN/Infinity) passes the None-only guard [app/workers/cost_reconciler.py:119-122].** _(FIXED — after `Decimal(str(cost))` coercion, `not cost_dec.is_finite()` → `_CostNotYetComputed` (retry), never a write. Empirically verified PG `Numeric(12,6)` accepts NaN as authoritative (corruption) and rejects Infinity (overflow crash); both now blocked. Unit tests added for NaN and Infinity.)_ The completeness gate rejects only `total_cost is None`. Verified empirically: `Decimal("NaN")` sums cleanly and persists into `Numeric(12,6)` as an authoritative NaN money value (silent corruption, never re-reconciled); `Decimal("Infinity")` raises PG `numeric field overflow` → uncaught task crash. Realistically low-probability (langsmith 0.10.10 types `total_cost` as `Optional[Decimal]`, so a non-finite value requires the LangSmith server to emit one), but the defense is a one-line guard: after coercion, treat a non-finite `Decimal` (`not cost_dec.is_finite()`) as incomplete → `_CostNotYetComputed` (retry), never a write.

- [x] [Review][Defer] **No metric/Sentry breadcrumb on `cost_reconcile_dispatch_failed` [app/workers/execution_tasks.py:948-960]** — deferred, observability gap (not a correctness bug). A broad broker outage silently converts a batch of completed rows to permanent estimates with only a log line; there is no counter/alert to make a systemic enqueue outage observable. Estimates are real dollars (AD-6 posture), so this is monitoring hardening, not a defect in this change.

- [x] [Review][Defer] **Empty/malformed-trace-after-full-retry is indistinguishable from LangSmith latency [app/workers/cost_reconciler.py:114-115, 245-247]** — deferred, diagnosability only. A trace that returns zero runs for its whole retry budget ends in `gave_up`, the same terminal state as a genuine "cost not ready yet." A future writer storing a malformed/non-trace id would be invisible. Log/count the "zero runs after full retry budget" case distinctly from "runs present but cost still None." Not caused by this change.

_Dismissed as noise (verified false-positive or by-design, not written above): float/bool `str()`-coercion concerns (moot — `Run.total_cost` is `Optional[Decimal]`, never float/bool from the real SDK); >6-decimal silent rounding (cost values are already at cost granularity; identical to how `pricing.py` estimates persist); transient-DB-error-not-retried (verified consistent with the established `parse_document` retry-scope precedent — catches only its own pending exception; AD-6 outage posture covers it); config-flip-mid-flight `not_configured` skip (the `_tracing_enabled` gate is correct-by-design; AD-6 posture)._
...
## Change Log (Code Review — 2026-07-30)

- 2026-07-30: **Code review complete (3 parallel adversarial layers + triage) → status `done`.** 1 decision-needed, 2 patch, 2 defer, 4 dismissed. All decision + patch findings resolved and re-tested in-container; no unresolved high/medium.
  - **[Decision→Patch, HIGH] Partial-trace undercount (AD-4/AD-8).** The completeness gate only checked "every *visible* llm run is priced," never "all the trace's llm runs are visible" — and `list_runs` is eventually-consistent, so a multi-LLM invocation could sum a partial child set and write it authoritatively, losing a not-yet-indexed child's cost forever (candidate re-check keys on `cost_is_estimated=true`, so the row is never revisited). **Fixed (user chose Option 1):** new `_require_trace_complete()` gates on the trace's **root run** being present AND finalized (`is_root=True` probe, `end_time` set) before trusting the child set — the root is the last run to finalize, so a finalized root guarantees all children are emitted. Empirically confirmed `list_runs` supports `is_root=True` in langsmith 0.10.10. Tests: root-not-finalized + root-absent → retry (unit); still-running-trace → retry, no write (integration).
  - **[Patch] AC3 0-row CAS branch was untested.** The redelivery test seeded an already-`false` row and exited at the candidate re-check, never exercising the `already_reconciled`/`rowcount==0` CAS branch. **Fixed:** added an integration test that races a concurrent flip-to-`false` between the candidate re-check and the CAS UPDATE, asserting the UPDATE matches 0 rows → `already_reconciled` with the value never regressed. The compare-and-set token now has real coverage.
  - **[Patch] Non-finite `total_cost` guard.** The None-only guard let `Decimal("NaN")` through; empirically PG `Numeric(12,6)` STORES NaN as an authoritative (uncorrectable) money value, and Infinity crashes on write. **Fixed:** `not cost_dec.is_finite()` → `_CostNotYetComputed` (retry), never a write. Unit tests for NaN + Infinity added.
  - **[Defer ×2]** Logged to `deferred-work.md`: (1) emit a metric/Sentry breadcrumb (not just a log) on `cost_reconcile_dispatch_failed` so a broad broker outage is observable; (2) distinguish "zero runs after full retry budget" from genuine LangSmith latency in the give-up log. Both are observability hardening, not correctness bugs.
  - **[Dismissed ×4]** float/bool `str()` coercion (moot — `Run.total_cost` is `Optional[Decimal]`); >6-decimal rounding (identical to `pricing.py` estimate persistence); transient-DB-error-not-retried (consistent with the established `parse_document` retry-scope precedent + AD-6 posture); config-flip-mid-flight `not_configured` skip (correct-by-design AD-6 posture).
  - **Gates (in-container, files `docker cp`'d + grep/sha-verified per the stale-baked-source memory):** `ruff check .` clean repo-wide; unit `test_cost_reconciler.py` 16 passed (was 12; +4), enqueue `TestMaybeEnqueueCostReconciler` 3 passed, integration `test_cost_reconciler.py` 10 passed (was 8; +2); full worker unit+integration 109 passed, 0 regressions. **Zero `docs/api-spec.json` diff** confirmed via proper in-container regen (`export_openapi.py` writes to file, not stdout — the transient "diff detected" was a stdout-capture artifact of stray structlog lines). `cost_reconciler` confirmed absent from the API import graph (worker-only). velara-web confirmed clean (no out-of-scope bundle). Task registers in the worker (`celery.tasks`).
