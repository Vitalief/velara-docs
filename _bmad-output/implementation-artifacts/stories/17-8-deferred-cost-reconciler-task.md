...
- [ ] **Task 1 — New reconciler Celery task module** (AC: 1, 2, 3, 4, 6, 7)
  - [ ] Create `app/workers/cost_reconciler.py` with a task decorated
  ...
  - [ ] **Session pattern (mandatory):** the task is a **sync** Celery function that bridges to async via
  ...
  - [ ] **Candidate re-check (AD-9):** load the one `InvocationResult` for the job (via
  ...
  - [ ] **Read-only LangSmith Client (AC7):** build a **dedicated** `langsmith.Client(api_key=settings.LANGSMITH_API_KEY)`
  ...
  - [ ] **Completeness gate + sum (AC4, AC1):** if the list is empty OR any run's `total_cost` is
  ...
  - [ ] **Compare-and-set UPDATE (AC3):** issue a single
...
- [ ] **Task 2 — Register the task module** (AC: 8)
  - [ ] Add `"app.workers.cost_reconciler"` to the `include=[...]` list in `app/workers/celery_app.py`
...
- [ ] **Task 3 — Enqueue-after-commit from the completion path** (AC: 5, 6)
  - [ ] In `app/workers/execution_tasks.py`, after `mark_completed`/`mark_blocked` returns (its commit has
  ...
  - [ ] **Broker-failure handling (AC5) — swallowing is MANDATORY, not tidy.** Wrap the dispatch in
        ...
        **swallowed** (the job is already complete and its estimate stands; do NOT rollback the cost row,
        ...
        not copy that here).
  - [ ] **Do NOT** enqueue for fan-out parents or `code` runs (they have `langsmith_run_id is None` → the
        `is not None` guard already excludes them — AD-9), and **do NOT** enqueue on the tracing-off path
        (`langsmith_run_id is None` — AD-6).
- [ ] **Task 4 — Confirm no hot-path / read-consumer / schema drift** (AC: 8)
  - [ ] Do **not** modify `app/core/pricing.py`, `_extract_cost_fields`, `app/services/analytics_service.py`,
  ...
  - [ ] `git status --short` after all edits shows only: `app/workers/cost_reconciler.py` (new),
        `app/workers/celery_app.py` (include), `app/workers/execution_tasks.py` (enqueue), and test files.
  - [ ] `python scripts/export_openapi.py` → zero `docs/api-spec.json` diff (AC8).
- [ ] **Task 5 — Tests** (AC: all)
  - [ ] **Unit — the reconciler task** (`tests/unit/workers/test_cost_reconciler.py`, new). Mock
  ...
  - [ ] **Integration — DB-backed CAS + no-flip-back** (`tests/integration/workers/test_cost_reconciler.py`,
  ...
  - [ ] **Enqueue-after-commit (AC5)** — a unit/integration assertion in the completion path
