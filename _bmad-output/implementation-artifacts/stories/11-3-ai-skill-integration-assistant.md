...
- [x] **Task 5 — FE review screen (AC3) — new, isolated feature slice**
...
- `docker compose run --rm -e AUTH_BACKEND=dev api python -m pytest` (post-implementation, in-container): 1210 passed, 3 failed — the 3 failures are the known pre-existing `test_ingest.py` MinIO-in-container failures (baseline unaffected; net +20 from 11.2's 1190/1210 lineage: 11 new unit + 9 new integration tests).
- `docker compose run --rm api ruff check .`: clean, no findings.
- `AUTH_BACKEND=dev .venv/bin/python scripts/export_openapi.py` (host): diff is exactly one new endpoint (`POST /api/v1/skills/integration-assistant/propose`) + its two new schemas (`AdapterProposalRequest`, `AdapterProposalResponse`) + the derived `ResponseEnvelope_AdapterProposalResponse_` wrapper — no other endpoint/schema changed.
- FE: `npm run typecheck` 0 errors; `npm run lint` 1 pre-existing `Icon.tsx` warning (baseline, unchanged); `npm run test` 605 passed (595 baseline + 10 new: 4 `AIAdapterReview.test.tsx` + 6 `SkillForm.test.tsx` additions).

### Completion Notes List

- **Task 1** — New `app/services/skill_integration_assistant.py`: pure `propose_adapter(*, llm_provider, files, entrypoint, manifest) -> AdapterProposal` function (no DB/session/storage args). Reuses `resolve_entrypoint_module` + `_signature_from_ast` from `entrypoint_contract.py` to build a structured signature description handed to the LLM alongside the entrypoint module's source only (not the whole bundle). Response parsed via two labeled fenced code blocks (`# ADAPTER` / `# MANIFEST`), never JSON-schema structured outputs, per the story's explicit guidance; malformed/unparseable responses raise `AdapterProposalParseError` (fail closed). The proposed adapter is pre-flight-checked with the SAME `validate_entrypoint_contract` registration uses; a non-conforming AI proposal sets `proposal_non_conforming=True` + a detail rather than being silently accepted. `core_files_unchanged` excludes BOTH the entrypoint module (the file just analyzed) AND the manifest path (resolved via `bundle_extractor.find_manifest_path`, not just a bare `"manifest.json"` literal, so a single-root-wrapped bundle's nested manifest is still correctly excluded) — this was caught by a failing unit test during development and fixed before the first full test run.
- **Task 2** — New `POST /api/v1/skills/integration-assistant/propose` endpoint in `skills.py`, gated with a per-route `dependencies=[RejectNonGrantor]` (admin/ma_tech only) — additive and scoped to this route only, since the router's existing `RejectClient` blanket gate is looser (consultant can still reach `create_skill`/`presign_bundle`, unchanged). Added an `Llm` FastAPI dependency alias in `dependencies.py` (mirrors the existing `SkillStorage`/`Secrets` pattern) wrapping `get_llm_provider()`, so the endpoint receives the provider via `Depends()` rather than a bare module-level call — this is also what makes the dependency swappable via `app.dependency_overrides` in the integration tests. Both the ZIP extraction/AST-parse (CPU-bound) and the LLM `.complete()` call (blocking network I/O via the sync Anthropic SDK) are wrapped in `run_in_threadpool` so the event loop is never blocked. New `EVENT_ADMIN_SKILL_ADAPTER_PROPOSED = "admin.skill_adapter_proposed"` audit constant; audit write is best-effort (try/except + `logger.warning`) after the response is built, mirroring `skill_service.py:923-946` exactly.
- **Task 3** — `CodeDrivenHybridManifest` gained an additive optional `ai_adapted: bool = False` field; confirmed Pydantic's default `ignore` extra-field behavior meant the field would otherwise be silently dropped (not "passed through") if left undeclared, so it had to be an explicit model field, not just relying on tolerance. No new backend "approve" endpoint — the FE assembles the ZIP and re-posts to the EXISTING `create_skill`/`create_version` bundle endpoints unchanged, so Story 11.2's `validate_entrypoint_contract` re-check applies to the approved artifact exactly as it would to any other bundle (verified by an integration test that intentionally breaks the "approved" adapter and confirms it still 422s `ENTRYPOINT_CONTRACT_VIOLATION`).
- **Task 4** — No code change, as expected: added an integration test that registers an AI-adapted bundle (manifest `ai_adapted: true`) and confirms the SAME `CERTIFICATION_INCOMPLETE` 422 fires on an uncertified `client_ready` transition attempt as for any hand-registered skill — pure regression proof that `assert_certified_for_client_ready` is untouched and unbypassed.
- **Task 5** — New, isolated `AIAdapterReview.tsx` (not a `SkillForm`/`SkillBundleUpload` rewrite). **Scope decision surfaced to the user and approved**: implementing the client-side ZIP reassembly on Approve required a browser ZIP library, which did not exist in `velara-web`; added `fflate` (~8KB, zero-dep) as a new FE dependency after asking the user, rather than hand-rolling a ZIP encoder or silently picking one. `SkillBundleUpload.tsx` gained a new optional `onFileChange` prop (backward-compatible, existing callers unaffected) so the parent can retain the raw `File` for later re-extraction of unchanged member bytes — the propose response only ever carries paths + sha256 checksums, never bytes (IP boundary), so the original bytes have to come from the client's own prior upload. `SkillForm.tsx`'s entry point reads the staged bundle's `manifest.json` entrypoint client-side (best-effort, never a security/validation boundary — the server independently re-derives everything from the bundle it fetches) to populate the propose request; the affordance is gated on `apiCode === 'ENTRYPOINT_CONTRACT_VIOLATION'` using the shared `getApiCode` helper (replacing the file's prior inline destructuring, per the story's explicit guidance) AND on a bundle file being present, so it can never appear for the inline-content (non-bundle) create path.
- **Task 6** — Full test coverage added per the story's exact list: BE unit (11 tests, including the AC5 golden behavioral-equivalence test that executes the LLM-proposed adapter source against the identical matrix `test_adapter_shim.py` uses), BE integration (9 tests: propose 200, RejectNonGrantor rejects both client AND consultant, missing-staged-bundle passthrough 422, audit-recorded, audit-best-effort-on-failure, approve-flow 201 with `ai_adapted: true` persisted in the stored manifest, approve-flow still 422s a broken AI adapter, cert-gate regression), FE (`AIAdapterReview.test.tsx` 4 tests, `SkillForm.test.tsx` +6 tests covering the affordance's exact gating conditions). One test-infra gap discovered and fixed: jsdom 25's `File`/`Blob` predate `arrayBuffer()` — added a `FileReader`-based polyfill to the shared `src/test/setup.ts` (affects any future File-content test, not just this story).
- **Task 7** — All gates green: `ruff check .` clean; full in-container suite 1210 passed (only the 3 known pre-existing `test_ingest.py` MinIO failures, unchanged baseline); `export_openapi.py` diff is exactly the one new endpoint + its 2 schemas, nothing else; FE typecheck/lint/test all clean at the expected baseline + new tests. No migration — `ai_adapted` lives entirely in the bundle's stored `manifest.json` blob, never a DB column, confirmed by fetching the stored manifest back from object storage in the approve-flow integration test rather than asserting against a DB row.

### File List

**velara-api:**
- NEW `app/services/skill_integration_assistant.py`
- NEW `tests/unit/services/test_skill_integration_assistant.py`
- MODIFY `app/api/v1/skills.py` — new `POST /api/v1/skills/integration-assistant/propose` endpoint
- MODIFY `app/core/dependencies.py` — new `Llm` dependency alias (`_llm_provider` + `Depends`)
- MODIFY `app/schemas/skill.py` — new `AdapterProposalRequest` / `AdapterProposalResponse`
- MODIFY `app/models/audit.py` — new `EVENT_ADMIN_SKILL_ADAPTER_PROPOSED` constant
- MODIFY `app/services/code_driven_hybrid.py` — new optional `ai_adapted: bool = False` manifest field
- MODIFY `tests/integration/api/test_skills.py` — 9 new Story 11.3 tests
- MODIFY `docs/api-spec.json` — regenerated (one new endpoint + 2 schemas, expected diff)

**velara-web:**
- NEW `src/features/skills/components/AIAdapterReview.tsx`
- NEW `src/features/skills/components/AIAdapterReview.test.tsx`
- MODIFY `src/api/skills.ts` — new `proposeSkillAdapter` client function + `AdapterProposal` type
- MODIFY `src/features/skills/components/SkillBundleUpload.tsx` — new optional `onFileChange` prop
- MODIFY `src/features/skills/components/SkillForm.tsx` — AI-adapt affordance entry point + `bundleEntrypoint`/`bundleFile` state
- MODIFY `src/features/skills/components/SkillForm.test.tsx` — 6 new tests for the affordance gating
- MODIFY `src/test/setup.ts` — `File.prototype.arrayBuffer` polyfill for jsdom
- MODIFY `package.json` / `package-lock.json` — new dependency `fflate`

## Change Log

- 2026-07-08 — Implemented Story 11.3 (AI Skill Integration Assistant): propose-only backend service + endpoint, unchanged-registration approve flow, certification-gate regression proof, FE review screen. Added `fflate` as a new FE dependency (user-approved) for client-side ZIP reassembly on approve. All gates green: BE 1210 passed in-container (3 known pre-existing failures only), ruff clean, api-spec.json diff is exactly the one new endpoint; FE typecheck 0 / lint baseline / vitest 605 (+10).
