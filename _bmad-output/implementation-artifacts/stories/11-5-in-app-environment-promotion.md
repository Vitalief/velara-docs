...
- [x] [Review][Patch] Add `test_promote_audit_survives_forced_audit_failure` [velara-api/tests/integration/api/test_skills.py] — `_audit_promote` mirrors the proven-safe best-effort `try/except Exception` pattern (`skill_service`'s canonical block), and export/import already have `test_export_import_audit_survives_forced_audit_failure` proving that pattern survives a forced audit-write failure. No equivalent test was added for promote's own `_audit_promote` wrapper, despite Task 8's test list. Low risk (same pattern, already proven elsewhere) but an unambiguous, cheap gap to close: monkeypatch `audit_service.record_admin_action` to raise, call `/promote` against a `client_ready` skill, assert the 422 `PROMOTION_NOT_CONFIGURED` response still returns normally. Applied 2026-07-14 — full BE suite 1343 passed / 3 skipped (baseline 1342 + 1 new test).
- [x] [Review][Defer] `SkillPromoteResponse` success path is entirely untested [velara-api/app/api/v1/skills.py:369-373] — deferred, pre-existing scope boundary. No test exercises a `PromotionResult` success response shape (`promoted_skill_id` non-null) because only `DisabledPromotionProvider` ships in this story — every request today terminates in `PROMOTION_NOT_CONFIGURED` before that code runs. Correct to defer to the Phase-2 story that adds `RemotePromotionProvider`, per this story's explicit L1 scope lock ("no transport this story").
- [x] [Review][Defer] Non-`PromotionNotConfiguredError` exceptions from `promotion.promote(...)` skip the audit write [velara-api/app/api/v1/skills.py:339-357] — deferred, pre-existing scope boundary. The `try/except PromotionNotConfiguredError` only catches that one type; any other exception from the provider would propagate without writing `_audit_promote`. Not reachable today — `DisabledPromotionProvider` only ever raises `PromotionNotConfiguredError` — but worth a broader `except Exception` + re-raise wrapper when `RemotePromotionProvider` (with real network/timeout failure modes) ships in Phase 2.
...
- 2026-07-14 — Code review (bmad-code-review, 3-layer: Blind Hunter + Edge Case Hunter + Acceptance Auditor). **Acceptance Auditor independently re-verified all 8 traps the story's own completion notes claimed were fixed — directly against the repo, not the narrative — and found zero AC violations.** 1 patch applied: `test_promote_audit_survives_forced_audit_failure` added (mirrored the existing export/import audit-failure-survival test; promote's own `_audit_promote` had no equivalent coverage despite the same best-effort try/except pattern). Full BE suite re-run post-patch: 1343 passed / 3 skipped (baseline 1342 + 1). 2 findings deferred as genuinely out of this story's L1 scope (no transport built yet): the `SkillPromoteResponse` success-path schema is untested (unreachable until `RemotePromotionProvider` ships), and non-`PromotionNotConfiguredError` exceptions from the provider would skip the audit write (not reachable with `DisabledPromotionProvider`, the only impl this story ships). Remaining reviewer findings (TOCTOU speculation, org-modeling questions, FE/BE role-string duplication, DAG-topology enforcement) dismissed as noise — either explicitly deferred by the spec itself or matching pre-existing codebase convention. Status → done.
