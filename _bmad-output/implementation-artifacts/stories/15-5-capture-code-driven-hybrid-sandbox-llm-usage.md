...
- [x] **Task 4 — Declare `reports_usage` on the manifest + route a non-declaring skill into the existing adapter flow (AC4) — `app/services/code_driven_hybrid.py` + `app/services/skill_service.py` + `app/services/skill_integration_assistant.py`**
  ...
  - [x] Resolved the `reports_usage: false` vs absent decision: the declared-as-bool check makes `false` a valid honest declaration ("skill makes no LLM calls" → registers) while an OMITTED (or non-bool, per review) key is the "author forgot" gap → assist. A declared `true` never routes to assist. Documented in `_manifest_missing_synthesizable_field`'s docstring + tested (`test_reports_usage_false_is_an_honest_declaration_not_a_gap`). ~~priced $0-code~~ CORRECTED in review: a declared-`false` run still prices NULL absent usage (pricing cannot distinguish declared-`false` from a legacy default-`False` at runtime); explicit $0-code needs a tri-state parse — deferred, see Review Findings + deferred-work.md.
...
- [x] [Review][Decision→Patched] `reports_usage: false` documented "priced $0-code" but nothing implements it — runs price NULL — RESOLVED (doc-fix + defer): the naive fix (thread parsed `reports_usage` → price $0 when False) would fabricate $0 for every LEGACY registered skill, because an old manifest without the key also parses to the model default `False` — indistinguishable from declared-`false` at runtime. Docstrings (`code_driven_hybrid.py`, `skill_service.py`, bundle test) and Task 4's note corrected to state declared-`false` runs still price NULL; the real $0-code implementation requires a tri-state parse (`bool | None`) and is tracked in deferred-work.md.
- [x] [Review][Decision→Patched] Assistant force-stamped `reports_usage: true` with no usage-emitting capability + could flip a client's declared `false` — RESOLVED (flip-fix + prompt instructions): the `propose_adapter` guard now prefers the CLIENT'S original declaration (`existing_manifest_data` on the synthesis path, the parsed manifest on the 11.3 path) before defaulting `True`; both `_SYSTEM_PROMPT` and `_SYNTHESIS_SYSTEM_PROMPT` now instruct adapters to lift client-exposed token usage into a top-level envelope `usage` key (never invent counts) and to declare `reports_usage` honestly in the updated manifest; the false "pairs this with an adapter that emits envelope usage" comment corrected. ⚠️ OPERATOR: the prompt additions need a live-LLM smoke test (a real propose → approve round-trip) — not verifiable in this offline review.
- [x] [Review][Decision→Patched] Inline/in-UI manifest authoring paths bypassed the `reports_usage` gate — RESOLVED (hard-422): new `_require_inline_reports_usage_declaration` raises `INVALID_CODE_DRIVEN_MANIFEST` with an explicit message at all 3 inline sites (create_skill / create_version / draft-update); inline paths have no staged bundle so assist-routing is impossible — the author just adds the key. Unparseable bytes still fall through to the parser's own errors. +2 tests.
- [x] [Review][Patch] Partial usage fabricates the forbidden $0 (+ None-key leakage into audit metadata and `pricing_unrecognized_model` log pollution) — FIXED: executor threads ONLY non-None usage fields; `_extract_cost_fields` treats an incomplete report (model or either count missing) as unpriceable → NULL, never `compute_cost_usd`'s None→0 coercion, and never the model=None triage-log false alarm. +1 integration pricing test (`test_cost_null_for_code_driven_hybrid_with_partial_usage`), threading test now asserts ABSENT keys. [velara-api/app/services/code_driven_executor.py:756, velara-api/app/workers/execution_tasks.py:180]
- [x] [Review][Patch] Skill-controlled token counts unbounded — FIXED: `Field(ge=0, le=2_147_483_647)` on both counts (+ validation test) [velara-api/app/services/code_driven_executor.py:22-24]
- [x] [Review][Patch] `"reports_usage": null` (or any non-bool) passed the presence gate then dead-422'd — FIXED: gate is now `not isinstance(data.get("reports_usage"), bool)` (+ test) [velara-api/app/services/skill_service.py:503]
- [x] [Review][Patch] No observability for `reports_usage: true` + usage-less envelope — FIXED: `code_driven_usage_contract_violation` warning (fires on usage absent OR both counts None; never for declared/legacy False) (+2 log tests) [velara-api/app/services/code_driven_executor.py]
- [x] [Review][Patch] Stale "not yet / forthcoming usage-contract story" comments — FIXED, rewritten to present tense [velara-api/app/workers/execution_tasks.py:126-133]
- [x] [Review][Patch] `_process_bundle` docstring omitted `reports_usage` from the MalformedBundleManifestError causes — FIXED [velara-api/app/services/skill_service.py:511]
- [x] [Review][Patch] Fan-out parent roll-up `cost_usd` never asserted (AC6 3-of-4) — FIXED: `_drive_fan_out` replica now mirrors the real `aggregate_results` seam (`**_extract_cost_fields`), and `test_fan_out_aggregates_to_completed` asserts the parent row's explicit `$0` [velara-api/tests/integration/api/test_invocations.py]
- [x] [Review][Patch] Misnamed test helper `_complete_manifest_without` — FIXED: renamed `_complete_manifest_with` + docstring [velara-api/tests/unit/services/test_skill_service_bundle.py]
- [x] [Review][Patch] Exact-model-id requirement documented in the `CodeDrivenUsage` docstring (skill-supplied ids must match pricing-table keys verbatim; no normalization) [velara-api/app/services/code_driven_executor.py:102]
- [x] [Review][Defer] Skill-supplied dated/aliased model ids (e.g. `claude-opus-4-8-20260115`) price NULL — no normalization before the exact-key pricing-table lookup [velara-api/app/core/pricing.py:83] — deferred, pricing-table exact-key policy is by design; revisit if real skills pin snapshot ids
- [x] [Review][Defer] `CodeDrivenUsage` has no cache-token fields (`cache_creation_input_tokens`/`cache_read_input_tokens`) — a conforming cache-heavy skill structurally underreports spend [velara-api/app/services/code_driven_executor.py:102-114] — deferred, future contract extension
...
- 2026-07-21 — Code review complete (3-layer: Blind Hunter + Edge Case Hunter + Acceptance Auditor). 12 patches applied (3 from resolved decisions), 4 deferred, 6 dismissed. HEADLINE (all 3 layers independently): partial usage (model without token split — a shape the new docstring invites) threaded None token values as PRESENT keys → skipped the NULL branch → `compute_cost_usd`'s `None or 0` fabricated the exact $0 this story exists to kill; fixed at both the executor (non-None-only threading) and the pricing seam (incomplete report → NULL). Also fixed: unbounded skill-controlled token counts (ge/le), null/non-bool `reports_usage` dead-422, missing contract-violation observability, inline-authoring gate bypass (hard 422), assistant `false`→`true` flip + usage-emission prompt instructions (⚠️ operator owes a live-LLM propose smoke test), fan-out roll-up cost assertion (closes AC6 4-of-4), stale comments/docstrings. `reports_usage: false` → "$0-code" claim corrected to NULL (tri-state parse deferred — naive impl would fabricate $0 for legacy skills). Gates: full suite 1514 passed / 1 pre-existing flake / 3 skipped; ruff clean; no api-spec diff. velara-api committed post-review: 6024f2f (branch development, not pushed). Status → done.
