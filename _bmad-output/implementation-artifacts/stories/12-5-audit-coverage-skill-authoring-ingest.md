...
_Code review 2026-07-13 (3-layer adversarial: Blind Hunter + Edge Case Hunter + Acceptance Auditor). 4 patch, 2 decision-needed, 3 defer. VERIFIED SOUND (reviewers actively tried and failed to break these): no double-emit on any create_skill path (derive + inline-import pass emit_audit=False; bundle-shape calls _create_skill_from_bundle directly, which never audits — count-based tests prove 1 derived/0 created, 1 imported/0 created); every audit write is strictly AFTER the mutation's commit so it can never roll one back; expire_on_commit=False means a swallowed audit failure cannot poison the session; ingest metadata leaks NOTHING forbidden (no filename/hash/bytes/parsed text); the AC4 guard is genuinely live-route-driven (independently enumerated: 42 live mutating routes vs 42 registry entries, 0 missing / 0 stale) and the 16 exemptions are legitimate + tracked in deferred-work.md; zero migration, zero api-spec diff._

_⚠️ SEPARATELY: CI was RED on main before this story (run 29237374763, commit 0b9606a — 3 failed / 1309 passed) and 12.5 would have added 2 more red tests. Root cause + fix are recorded in the CI section below — NOT a 12.5 defect, but 12.5's 2 new ingest tests were also failing under it._

- [x] [Review][WITHDRAWN — NOT A DEFECT] "Client-portal ingest audit rows are written into an org no principal can read" [velara-api/app/services/ingest_service.py:367] — WITHDRAWN by the Project Lead on review, and correctly so. `org_id` IS the tenant column (`audit_service.py:66` says so verbatim): `org_client_001` is a SEPARATE TENANT from `org_vitalief`, not a sub-org. Writing the client's ingest audit row under the client's own `org_id` is therefore CORRECT tenant isolation — it is the only place it may be filed. The Edge Case Hunter mistook "no *internal* reader exists today" for "unreadable by anyone", and every remedy floated (key the event to the auditing org / widen the read-fence to grant-reachable orgs / dual-write) would have PUNCHED A HOLE IN TENANT ISOLATION by leaking one tenant's audit trail into another's. There is deliberately NO cross-tenant auditing in this product. The row is correctly filed under its tenant and will be read through the same org fence if/when a client-facing audit surface is built. NO CHANGE. ⚠️ LESSON for future reviews: verify the tenancy model BEFORE proposing anything that moves or widens an `org_id` fence.
- [x] [Review][Decision→PATCH APPLIED] ✅ `update_skill_metadata` writes unbounded, unredacted skill IP into the append-only audit table [velara-api/app/services/skill_service.py:1575-1583] — `changed_fields` records old→new for EVERY patched field including `description`, `input_schema`, `output_schema`, `consumes`, `tags`. `_json_safe` only `str()`-ifies; it truncates nothing and redacts nothing. VERIFIED EMPIRICALLY: one PATCH with a long description + a 200-key input_schema wrote an ~11KB `changed_fields` blob, stored verbatim and permanently. The same diff explicitly refuses to log `original_filename`/`content_sha256` for ingest on the rationale "append-only; a leaked value can never be deleted" — yet dumps arbitrary skill-internal contract text into that same table. Also `str(dict)` yields Python repr (single quotes / None / True), not JSON, so the JSONB column holds a non-queryable string and the FE renders raw repr. The story SANCTIONED `str()` for non-primitives, so overriding it is a product call. DECISION: (a) log only the changed KEY NAMES (the existing logger.info already does this), (b) old→new only for small enum-ish fields (visibility/scope/output_format/location_dependent) + a redaction marker for free-form/dict fields, or (c) accept as-is.
- [x] [Review][Patch] ✅ APPLIED — Ingest audit attributes the action to the PRESIGNER, not the confirming user [velara-api/app/services/ingest_service.py:366] — `user_id=ref.created_by_user_id` records whoever called `/presign`, not whoever called `/confirm`. `_get_file_ref_or_404` scopes only by `org_id` (never by user), so ANY user in the org can confirm another user's pending upload and the append-only audit permanently names the wrong actor. VERIFIED EMPIRICALLY: usr_004_admin presigns → usr_001_ma_tech confirms → the row reads `user_id = usr_004_admin`. Both routers already have `user: CurrentUser` in scope (ingest.py:93, client.py:636) and simply don't pass it; the code comment concedes the shortcut. The 2 new tests miss it because they presign and confirm as the same principal. FIX: add an `acting_user_id` param to `confirm_file_ref`, pass `user.user_id` from both routers, and add a cross-user-confirm test.
- [x] [Review][Patch] ✅ APPLIED (derive only) — `derive_skill`: a post-commit failure leaves a COMMITTED skill with ZERO audit events [velara-api/app/services/skill_service.py:1654-1704, skill_export.py:394-418] — `create_skill(emit_audit=False)` COMMITS the child skill, then the second lineage transaction can raise (DB error on `paired_with`/`derived_from`) → `rollback()` + `raise`, never reaching the `admin.skill_derived` write. Net: a live, committed `client_facing` skill with no `skill_created` (deliberately suppressed) and no `skill_derived` (never reached) — precisely the unaudited-mutation blind spot this story exists to close, reintroduced by the suppression. Same shape in `import_skill_bundle` (its `skill_imported` write is best-effort-swallowed, so an audit failure there also yields a committed skill with zero events). FIX: emit a fallback event in the failure path, or don't suppress `skill_created` until the specific replacement event is durably written.
- [x] [Review][Patch] ✅ APPLIED — `update_draft_content` records no content identity — a no-op re-save is indistinguishable from a real edit [velara-api/app/services/skill_service.py:1421-1436] — the event metadata is only `{skill_id, version, is_bundle}`. The stated reason for auditing this call is content provenance ("it changes content with no prior who/when record"), yet the event carries neither the old nor the new `artifact_checksum` — both already in scope at the call site (`old_key`, `checksum`, `current_ver.artifact_checksum`). An auditor cannot tell whether the bytes actually changed, nor reconstruct the draft sequence. Contrast `update_skill_metadata`, which DOES record old→new. FIX: record `{"from_checksum": <old>, "to_checksum": <new>}` — checksums, not content, so no IP leak.
- [x] [Review][Patch] ✅ APPLIED — The bundle-shaped `create_skill` audit exit has ZERO test coverage [velara-api/app/services/skill_service.py:768-776] — Task 6 claims "full behavior-test coverage", but every `admin.skill_created` assertion goes through the INLINE exit. The only bundle-path test exercises the `emit_audit=False` branch, so the `if emit_audit:` block that asserts `is_bundle: True` is never executed by any test (grepped: no test asserts `event_metadata["is_bundle"] is True` for `EVENT_ADMIN_SKILL_CREATED`). The code looks correct, but the story's own thesis is "a coverage invariant with no test to enforce it". FIX: one test — POST /skills with a `bundle_key`, assert exactly one `admin.skill_created` with `is_bundle: True`.
- [x] [Review][Defer] FE completeness "guard" is a hand-copied list and cannot catch the drift it exists to prevent [velara-web/src/features/audit/eventTypeIconMeta.test.ts:14-36] — deferred, SPEC-SANCTIONED. `ALL_EVENT_TYPES` is a hand-maintained duplicate of the Python `EVENT_*` constants; the test only asserts every entry IN THAT LIST maps to a non-default icon. Add a new `EVENT_ADMIN_*` to `app/models/audit.py` and forget the TS list → the list and map stay mutually consistent, the test still passes, and the event silently renders as the default `play` icon — the EXACT failure mode that produced the 3 drifted 11.3/11.4 icons this story is fixing. The list is complete today (21/21 verified). The story explicitly blessed the maintained list as "the honest mechanism", so it is compliant — but AC6's claim of "closing 12.3's coverage for good, not just for today's five" is not actually achieved. Cheap closure: have the BE guard read `eventTypeIconMeta.ts` as text and assert every `EVENT_ADMIN_*` constant appears as a key.
- [x] [Review][Defer] Audit-coverage guard: registry can't express a multi-event route, and the route walk skips `Mount` sub-apps [velara-api/tests/unit/test_audit_coverage_guard.py] — deferred, latent only. (1) `POST /api/v1/access-grants` emits `grant_created` OR `grant_reaffirmed` by branch, but the registry records only one constant, so a future branch that stops emitting the second would not be caught. (2) `_live_mutating_routes` drops any `starlette.routing.Mount` (has `.routes` but no `.methods`) and everything inside it; `app/main.py` uses `include_router` exclusively today, so the walk is currently exact (independently verified 42/42). Both are future-proofing, not live gaps.
- [x] [Review][Defer] New event types have no audit-log filter pill — only reachable under "All events" [velara-web/src/features/audit/eventKindMeta.ts:31-40] — deferred, pre-existing + out of AC6 scope. `EVENT_KIND_OPTIONS`/`eventKindToParams` are unchanged by this diff and the API supports only single-value exact-match `event_type`, so all 6 new `admin.skill_*` / `admin.document_ingested` types are unfilterable. The file's own header already flags this limitation (the grants pill misses `grant_reaffirmed`/`grant_revoked`); this change grows the unreachable set from 2 to 8. AC6 covered icons, not discoverability.

### CI fix (not a 12.5 defect — a Story 11.4 regression surfaced during this review)

**CI was RED on `main` before this story and nobody knew.** Run 29237374763 (commit `0b9606a`): `3 failed, 1309 passed`. The standing team note — *"3 known pre-existing MinIO failures, LOCAL-ONLY, not a CI blocker"* — was **false** and is hereby RETIRED. Two distinct problems were hiding behind it:

**Root cause.** Story 11.4 added an in-memory fake storage provider in `tests/conftest.py` (active unless `USE_REAL_STORAGE=1`) to dodge uninterruptible C-level socket hangs on Docker Desktop for Mac. Its `presign_upload` returns a fabricated URL — `http://in-memory-storage.test/...`. But `test_ingest.py`'s module skip-gate only asks *"is MinIO reachable?"* — which in CI it **is** — so the module does NOT skip, and 3 tests then do a real `httpx.put()` to that non-resolvable host → `ConnectError`. The gate and the storage layer disagreed about which backend was live. The `pytest.skip` guards at those sites only inspected `put_resp.status_code`, so they could never fire: `httpx.put` raises a *transport exception* before any status exists.

Nobody caught it because **three commits (`bfcf591`, `5d61b11`, `4b79511`) landed on `main` without ever triggering a CI run** — the regression sat red for four days, and 12.5 would have added 2 MORE red tests (its own new ingest audit tests do the same PUT).

**Two fixes applied:**
1. `.github/workflows/ci.yml` — set `USE_REAL_STORAGE: "1"` in the test job. CI already runs a real MinIO with buckets created on a resolvable `localhost:9000`, so the presigned PUTs genuinely work: real end-to-end ingest coverage is RESTORED rather than skipped away. Local dev keeps the fast fake provider (11.4's Docker-Desktop fix is preserved).
2. `tests/integration/api/test_ingest.py` — new `_put_presigned()` helper wraps the 3 PUT sites and converts a transport failure into a clean `pytest.skip` with an actionable message. Defence in depth: a fake-storage/no-MinIO environment now SKIPS (as the module docstring always promised) instead of hard-failing.

**⭐ This also killed the "local wall".** The same 3 tests were what made a full local suite run blow up partway through — mistaken for "the test suite has gotten too big". It has not: **the full suite is 1289 tests in 43 seconds.** After the fix, a full local run is `1289 passed, 39 skipped, 0 failed` (43s), and under real storage (the CI path) all 15 ingest tests genuinely execute and pass.

**Process lesson:** a persistently-red test that gets explained away as "known/environmental" is a *bug in the explanation*, not a fact about the environment. Verify the claim against an actual CI run before trusting it — and never let commits reach `main` without CI.

### Post-patch verification (2026-07-13)
- **BE, full suite, local (fake storage):** `1289 passed, 39 skipped, 0 failed` in 43s — the local wall is GONE.
- **BE, ingest suite, real storage (the CI path):** `15 passed` — includes the 3 formerly-red tests, genuinely executing.
- **BE lint:** `ruff check .` (what CI gates) — **All checks passed.** (Note: `ruff format --check` reports ~66 pre-existing unformatted files repo-wide; CI does NOT gate on it, so it was left alone.)
- **FE:** `tsc --noEmit` 0 · eslint 0 · vitest **654 passed**.
- **AC5 re-verified after patches:** `git diff docs/api-spec.json` = EMPTY, zero migrations. `acting_user_id` is a service-layer kwarg, not a request field — the API contract is unchanged.
- **New tests added by this review:** cross-user confirm (proves the audit names the CONFIRMER); bundle-path `skill_created` (the untested audit exit); free-form redaction (proves skill IP is not persisted).
