...
_Code review 2026-07-22 (3-layer: Blind Hunter + Edge Case Hunter + Acceptance Auditor, all Opus). Blind Hunter and Acceptance Auditor INDEPENDENTLY found the same HIGH root cause: the AC7/AC8 repointing was applied to the COLLECTION paths (`list_locations`, fan-out children) but NOT to the SINGLE-location invocation path, which still scope-checks and stamps its job against the Location's own — now Client-rooted — `hierarchy_path`._

- [x] [Review][Patch] **HIGH — Single-location invocation wrongly 403s a Study/Project-scoped client (AC8 breach on the single-job path)** — `_resolve_single_job_hierarchy_path` does `scope.assert_in_scope(str(location.hierarchy_path))` against the now-Client-rooted location path. A `client`-role user with a Study-scoped grant runs a location-dependent single-location job via `POST /api/v1/client/invocations/{skill_id}` (client.py reuses `queue_invocation` verbatim, NOT `RejectClient`-guarded) → the Client-rooted path is not a descendant of their study grant → `HierarchyScopeError` 403 on a location their grant legitimately covers. This is the EXACT regression class AC8 exists to prevent, applied to the single-job path the fix missed. Masked by every client-invocation test seeding the grant at the CLIENT node. Fix: scope-check against the associated Study's path (mirror `list_locations`'s AC8 fix), not the raw location path. [velara-api/app/api/v1/invocations.py:208]

- [x] [Review][Patch] **HIGH — Single-location job stamped with the Client-rooted path (AC7 breach on the single-job path)** — `_resolve_single_job_hierarchy_path` returns `str(location.hierarchy_path)` as the job's `hierarchy_path` (invocations.py:217 → used at :375). AC7 fixed fan-out CHILD jobs to tag `str(study.hierarchy_path)` but left the single-location sibling on the Location's Client-rooted path. Consequence: the job is mis-scoped in `job_service.list_jobs`'s `hierarchy_path <@ ANY(scope_paths)` filter and in its audit entry — a Study-scoped client cannot see their own single-location job in history. Must be fixed together with the finding above (same resolution: derive the single-job path from the Study context). [velara-api/app/api/v1/invocations.py:217]

- [x] [Review][Patch] **MED — Migration omits the parity assertion Task 2 explicitly required (checkbox marked done but not in code)** — Task 2 specified "assert row-count parity before this step — every Location must resolve a client_id or the migration is unsafe to proceed." `0025` goes straight from the `client_id` backfill `UPDATE` to `op.alter_column(... nullable=False)` with only a comment; no actual guard. A Location with a broken study→project→client chain fails with an opaque NOT NULL violation instead of the specified explicit parity check. Add a `SELECT COUNT(*) ... WHERE client_id IS NULL` assertion (raising if non-zero) before the `SET NOT NULL`. [velara-api/app/db/migrations/versions/0025_location_client_ownership.py:107]

- [x] [Review][Patch] **LOW — `associate_location_to_study` concurrent-duplicate races to a 500, not 409** — the existence check (`session.get`) then `session.add`+`commit` has a TOCTOU window; two concurrent identical associates → the second hits an `IntegrityError` on the composite PK, surfacing as an unhandled 500 instead of the intended `LOCATION_STUDY_ASSOCIATION_EXISTS` (409). Other create paths in this file catch `IntegrityError` (see `_get_or_create_org`, `_commit_delete`). Wrap the commit in a `try/except IntegrityError` → rollback + raise `LocationStudyAssociationExistsError`. [velara-api/app/services/hierarchy_service.py:886]

- [x] [Review][Patch] **LOW — `downgrade()` aborts when any Location has zero Study associations** — a Client-created-but-never-associated Location is a normal post-migration state; on downgrade its `study_id` subquery returns NULL → `ALTER COLUMN study_id SET NOT NULL` fails. Downgrade is a schema-emergency path (AC2 already documents it as lossy for the multi-Study case), but the zero-association case should at least not hard-crash: either `DELETE` unassociated Locations first (documented) or leave `study_id` nullable on downgrade with a note. [velara-api/app/db/migrations/versions/0025_location_client_ownership.py:141]

_Dismissed (4, verified false-positive or not-actionable): (1) Admin GET/PATCH/DELETE `/locations/{id}` scope-check on the Client-rooted path (hierarchy.py:556/575/604) — NOT REACHABLE: the hierarchy router is `dependencies=[RejectClient]`, so only internal roles reach it, and all internal roles are `unrestricted=True` (scope bypass); `assert_in_scope` is a no-op there for every caller that can reach it. (2) Upgrade step-3 `subltree` assumes path depth ≥2 — every real pre-existing Location IS study-depth (the only shape `create_location` ever produced pre-16.1); fail-loud, not silent corruption; no malformed rows exist. (3) Fan-out `list_locations(study_id)` called without `scope_paths` (invocations.py:330) — scope is asserted on the STUDY first (line 329, `assert_in_scope(str(study.hierarchy_path))`), so not exploitable; pre-existing call shape. (4) associate(409)/disassociate(204) strictness asymmetry — deliberate idempotency, defensible design choice._
...
- 2026-07-22 — Code review complete (3-layer: Blind Hunter + Edge Case Hunter + Acceptance Auditor, all Opus). 5 patches applied, 0 deferred, 4 dismissed. ⭐ HEADLINE (found INDEPENDENTLY by Blind Hunter AND Acceptance Auditor): the AC7/AC8 hierarchy_path repointing was applied to the COLLECTION paths (`list_locations`, fan-out children) but MISSED the single-location invocation path — `_resolve_single_job_hierarchy_path` still scope-checked AND stamped its job against the Location's now-Client-rooted `hierarchy_path`. A `client`-role user with a Study-scoped grant running a single-location job via `POST /api/v1/client/invocations` (reuses `queue_invocation`, NOT RejectClient-guarded) was wrongly 403'd at submit, and its job would be mis-scoped out of their own history/audit — the exact AC8 regression class, on the one path the fix missed. Completely masked because every client-invocation test seeded the grant at the CLIENT node. Fixed both (HIGH x2) at the single seam: when a `study_id` is supplied, derive both the scope check and the job's hierarchy_path from the Study's path (mirroring the fan-out/list fixes); fall back to the Location's own path only when no study context is given. Added a study-scoped single-location regression test (`test_client_study_scoped_single_location_invocation_succeeds`) — verified test-has-teeth (fails 403 against the pre-fix code, passes after). Other 3 patches: (MED) migration's Task-2-required parity assertion was marked done but absent — added an explicit `COUNT(*) WHERE client_id IS NULL` guard before `SET NOT NULL`; (LOW) `associate_location_to_study` concurrent-dup race → 500 not 409, added `IntegrityError`→409 handling; (LOW) `downgrade()` crashed on a zero-association Location (a normal post-16.1 state) — now DELETEs unassociated orphans first (verified live: created an orphan, downgraded, orphan dropped + associated location correctly restored, then upgraded back with the parity guard passing). 4 dismissed (verified): admin GET/PATCH/DELETE scope-check (not reachable — hierarchy router is RejectClient + internal roles are unrestricted); step-3 depth assumption (no malformed rows exist); fan-out no-scope-filter (study scope asserted first); associate/disassociate strictness asymmetry (deliberate). Gates re-run: full suite 1548 passed / 2 pre-existing flakes (confirmed unrelated via isolated re-run) / 3 skipped; ruff clean; api-spec unchanged (all patches internal). Not committed to velara-api (subrepo).
