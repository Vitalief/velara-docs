...
_Code review 2026-07-16 (3 layers: adversarial, edge-case, acceptance-auditor). All 3 layers completed; strong convergence on the two apply-only defects. Severities reflect real-consequence-at-call-site, not the subagents' raw ratings._

- [x] [Review][Decision → RESOLVED: keep, intentional] **`terraform/bootstrap-nonprod/` is a state-backend/account migration that was undeclared in this story's scope.** [terraform/bootstrap-nonprod/main.tf] — The directory is byte-identical to the committed `terraform/bootstrap/main.tf` but targets a **different AWS account (669424048050)** than the original bootstrap (068858795262), provisions state bucket `velara-tfstate-ad0deb2a`, and was actually `terraform apply`'d (local `terraform.tfstate` at serial 8); the committed `backend.dev.hcl` was re-pointed to the new bucket. **Reviewer decision 2026-07-16: KEEP — confirmed intentional (a deliberate dev-account/backend migration).** The Dev Agent Record's "only `terraform/*.tf` detective-control files / plan-only" scope claims were inaccurate for this directory and have been corrected below (see Completion Notes addendum + File List) to record the backend/account migration and the fact that `apply` was run for the bootstrap module. No further code action on the directory itself.
- [x] [Review][Patch] CloudTrail `s3_key_prefix = "AWSLogs"` double-nests the delivery path (`AWSLogs/AWSLogs/<account>/…`) but the bucket policy grants `PutObject` only on `AWSLogs/<account>/*` → delivery denied / trail creation fails at apply (the exact Trap-3 silent-delivery-failure this story set out to avoid) [terraform/cloudtrail.tf:164] — **FIXED:** removed `s3_key_prefix`; CloudTrail now writes to its default `AWSLogs/<account>/…` path, which the existing policy already grants.
- [x] [Review][Patch] `aws_sns_topic_policy.alerts_guardduty` replaces the alerts topic's default access policy with an `events.amazonaws.com`-only statement, omitting `cloudwatch.amazonaws.com` → risks breaking all four existing alarm→SNS deliveries, including Story 13.4's `security_events` breach alarm [terraform/guardduty.tf:72] — **FIXED:** added `AllowCloudWatchAlarmsPublish` (cloudwatch.amazonaws.com, gated by SourceAccount) + `AllowAccountOwnerFullControl` statements so attaching the policy preserves alarm→SNS delivery and account ownership.
- [x] [Review][Patch] VPC flow-log IAM policy scopes `logs:DescribeLogGroups` to `${log_group.arn}:*`; that action is account/region-level and is denied at a stream ARN → flow-log role fails group discovery, flow logs never initialize [terraform/vpc.tf:158] — **FIXED:** split into two statements — stream-level write actions on `<group>:*`, `DescribeLogGroups` on the account-level `log-group:*` ARN.
- [x] [Review][Patch] ALB `access_logs` references `aws_s3_bucket.alb_logs` with no `depends_on` on `aws_s3_bucket_policy.alb_logs` → first-apply race: ALB validates its test-write before the bucket policy exists (CloudTrail correctly has this `depends_on`; ALB is the asymmetric miss) [terraform/alb.tf:25] — **FIXED:** added `depends_on = [aws_s3_bucket_policy.alb_logs]` to `aws_lb.main`.
- [x] [Review][Patch] `var.log_retention_days` is a bare `number` with no `validation` block, but feeds `aws_cloudwatch_log_group.retention_in_days` which accepts only a fixed enum (…,180,365,400,…). Default 365 is valid; any operator override to a non-enum value passes `validate`, fails at apply on the two log groups, yet silently succeeds on the S3 lifecycle rules — an asymmetric, non-obvious failure [terraform/variables.tf:192] — **FIXED:** added a `validation` block restricting the value to the CloudWatch-accepted retention enum.
- [x] [Review][Patch] CloudTrail KMS key policy grants no `kms:Decrypt` to any operator/CI role (only root `kms:*` + the CloudTrail write path). Reading the trail — the entire point of the detective control / SOC 2 evidence path — requires `kms:Decrypt`; the gha-deploy/consultant roles cannot read it [terraform/cloudtrail.tf:22] — **FIXED:** added `AllowAccountToDecryptTrailLogs` (Decrypt/DescribeKey for account IAM principals, gated `kms:ViaService = s3.<region>.amazonaws.com`) so IAM-permitted operator/CI roles can read the SSE-KMS trail.
- [x] [Review][Defer] CloudTrail bucket has no versioning/Object-Lock while log-file-validation advertises tamper-evidence — a delete-capable principal can still remove log objects [terraform/cloudtrail.tf:69] — deferred, hardening beyond AC scope
- [x] [Review][Defer] `aws_guardduty_detector.main` fails with `BadRequestException` if a detector already exists in the account/region (one-per-region limit); no data-source/import guard [terraform/guardduty.tf:10] — deferred, apply-time operator concern for pre-existing GuardDuty
- [x] [Review][Defer] ALB newer-region service-principal statement grants only `s3:PutObject`, omitting `s3:GetBucketAcl` that the `logdelivery.elasticloadbalancing.amazonaws.com` form also needs → delivery could fail in a region that uses the service principal (us-east-1 default uses the legacy account form, so latent) [terraform/cloudtrail.tf:274] — deferred, region-conditional, not triggered at the configured region
...
- `velara-api/terraform/bootstrap-nonprod/` — **NEW (out of the detective-controls scope; recorded by code review 2026-07-16).** A state-backend bootstrap root module (S3 state bucket `velara-tfstate-ad0deb2a` + `velara-tfstate-lock` DynamoDB table) targeting AWS account **669424048050** — a **deliberate dev-account/backend migration**, byte-identical in `main.tf` to the existing `terraform/bootstrap/` (account 068858795262). **This module WAS `terraform apply`'d** (local state at serial 8) — the story's "plan only" rule applies to the detective-control resources, NOT to this state-bootstrap module, which by nature must be applied out-of-band to exist before any backend can use it. `backend.dev.hcl` (gitignored) was re-pointed to the new bucket. `main.tf` + `.terraform.lock.hcl` are untracked; `terraform.tfstate` is gitignored.

  **Scope-claim correction:** the "only `terraform/*.tf` detective-control files modified/added" and blanket "plan-only, no apply" statements in the Completion Notes above are accurate for the CloudTrail/GuardDuty/ALB/flow-log/retention work but do NOT cover this bootstrap module. This addendum is the honest record.
