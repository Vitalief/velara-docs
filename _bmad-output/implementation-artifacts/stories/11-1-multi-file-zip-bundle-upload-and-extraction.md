...
**Code review 2026-07-08 (bmad-code-review, 3-layer adversarial: Blind Hunter + Edge Case Hunter + Acceptance Auditor).** Diff reviewed: velara-api `4ccdd31..5498943` + velara-web `d31f6ad..b725c98` (11.1 commits only; 11.8 excluded — pending its own review). **Outcome:** 1 decision (resolved → defer to 11.2), 16 patches ALL APPLIED same day, 2 deferred (→ deferred-work.md), 5 dismissed. Post-patch gates: BE ruff clean, 1138 passed in-container (+21 new tests; only the 3 known pre-existing test_ingest MinIO failures), api-spec.json unchanged (regen verified); FE typecheck 0 / lint baseline / vitest 595 passed (+4 new).

- [x] [Review][Decision → Defer] Bundle versions are publishable/runnable today and crash at run time — the executor (out of scope per Dev Notes; consumption is Story 11.2) does `skill_storage.get(current_ver.artifact_key)` on what is now a directory *prefix* for bundle versions (execution_service.py:429, 543, 698-700 — no `is_bundle` gate anywhere outside skill_service/models/schemas), and nothing blocks certifying/publishing/invoking a bundle skill. Running one → botocore NoSuchKey → unmapped generic job failure. **RESOLVED 2026-07-08 (user decision): accept + defer to 11.2** — executor consumption of bundles IS Story 11.2 (next in epic order); interim generic run-failure accepted rather than adding a throwaway guard.
- [x] [Review][Patch] 🔴 CRITICAL `bundle_key` accepted verbatim — cross-org read + arbitrary skill-bucket object deletion (IDOR). Presign mints `bundle-staging/{org_id}/{uuid}/...` but confirm never validates the submitted key's prefix or org; any internal user can register another org's staged bundle (or any committed `skills/...` object that parses as a ZIP) as their own skill, and the post-success `_safe_delete(storage, body.bundle_key)` deletes whatever client-named key was passed. Fix: reject any `bundle_key` not under `bundle-staging/{caller_org_id}/` (422) in both create + version paths. [velara-api app/api/v1/skills.py:86-99,283-293; app/services/skill_service.py fetch_staged_bundle]
- [x] [Review][Patch] HIGH Bundle path does not enforce `runtime_type == "hybrid"` — a `prompt`/`code` skill can be bundle-created (or bundle-versioned) and gets code-driven `schema_version`/`egress` metadata plus a prefix `artifact_key` its executor can never read; the cross-shape guard only fires for `runtime_type == "hybrid"`. [velara-api app/services/skill_service.py:492-493,923-925]
- [x] [Review][Patch] HIGH Duplicate member paths (exact, or colliding after backslash normalization) corrupt `artifact_set` — `data_by_path = dict(files)` keeps last-occurrence bytes for every write while `members` records each occurrence's own sha256 → recorded checksum mismatches stored bytes, violating AC1's re-extract-identical invariant. Fix: reject duplicate post-normalization paths as INVALID_BUNDLE. [velara-api app/services/bundle_extractor.py extract_bundle; skill_service.py:339-344]
- [x] [Review][Patch] HIGH `zf.read(info)` failures escape as 500 — only the ZipFile constructor is wrapped; a bad-CRC/truncated member raises BadZipFile (empirically verified) and an encrypted member raises RuntimeError, both uncaught → 500 instead of 422 INVALID_BUNDLE. [velara-api app/services/bundle_extractor.py:171]
- [x] [Review][Patch] HIGH No size cap on the staged ZIP itself — presigned PUT has no content-length constraint and `fetch_staged_bundle` loads the whole object into RAM before the magic-byte check (SKILL_BUNDLE_MAX_* caps bound uncompressed members only) → multi-GB upload = memory-exhaustion DoS. Fix: `storage.head()` size check before `get`. [velara-api app/services/skill_service.py fetch_staged_bundle]
- [x] [Review][Patch] HIGH FE `putBundleToS3` never checks `response.ok` — fetch resolves on S3 403/400/5xx, so a failed PUT reaches the "ready" state and hands the parent a key with no object behind it (surfaces later as an opaque BUNDLE_NOT_UPLOADED). The companion error test mocks an axios-shaped rejection fetch can never produce (asserts an unreachable path) — fix the client, rework the test, and cover the real confirm-side 422 render. [velara-web src/api/skills.ts:115-121; SkillBundleUpload.test.tsx]
- [x] [Review][Patch] MEDIUM Switching `runtime_type` away from hybrid strands a staged `bundle_key` — upload widget unmounts, textarea stays hidden (`!fields.bundle_key` guard is not hybrid-gated), no way to see/clear the key, and submit still sends it. Fix: clear `bundle_key` on runtime_type change. [velara-web src/features/skills/components/SkillForm.tsx]
- [x] [Review][Patch] MEDIUM Non-canonical member paths (`pkg/../x`, `./x`, `a//b`) pass `_is_path_safe` (checked on the collapsed form) but the RAW path lands in S3 keys and `artifact_set` — URL-normalizing clients can't address the objects and future filesystem joins re-open traversal. Fix: require canonical paths (reject when normpath ≠ path). [velara-api app/services/bundle_extractor.py:96-101]
- [x] [Review][Patch] MEDIUM `fetch_staged_bundle` bare `except Exception` maps S3 outages/timeouts to 422 BUNDLE_NOT_UPLOADED — narrow to not-found only (5.5.1 narrow-except lesson). [velara-api app/services/skill_service.py:417-420]
- [x] [Review][Patch] MEDIUM Full ZIP decompression + per-member SHA-256 runs synchronously on the event loop (up to 100 MB default cap) — move `_process_bundle`/hashing into `run_in_threadpool` like the S3 puts. [velara-api app/services/skill_service.py:355-368,467,926]
- [x] [Review][Patch] LOW Trailing-backslash member (`pkg\`) passes the directory check (endswith("/") tested before backslash normalization) → phantom zero-byte "file" stored at a directory-marker key. [velara-api app/services/bundle_extractor.py:85-91]
- [x] [Review][Patch] LOW `_sanitize_bundle_filename("..")` survives sanitization → staging key ends `/..` → browser URL-normalizes before PUT → signature mismatch 403. Reject dot-only names to the fallback. [velara-api app/services/skill_service.py:407-411]
- [x] [Review][Patch] LOW No member-path length cap — S3 caps keys at 1024 bytes; a long member path fails `storage.put` mid-loop → orphan-cleanup + 500 instead of a clean 422 at extraction. [velara-api app/services/bundle_extractor.py]
- [x] [Review][Patch] LOW Task 9 checkbox checked but unimplemented: no determinism/reorder-stability test exists for the per-file/bundle checksums (grep for `_write_bundle`/`bundle_checksum` in tests → zero hits). [velara-api tests/unit/services/test_bundle_extractor.py]
- [x] [Review][Patch] LOW Task 9 gap: oversize bundle is unit-tested only — the "422 INVALID_BUNDLE; nothing persisted" integration assertion for oversize was never written. [velara-api tests/integration/api/test_skills.py]
- [x] [Review][Patch] LOW Form can be submitted while the bundle upload is in flight (`bundle_key` still empty; late `onBundleKeyChange` fires into a possibly-submitted form) — expose an `onUploadingChange` and suspend submit, per the Story 12.4 DocumentUploadCard precedent. [velara-web SkillForm.tsx / SkillBundleUpload.tsx]
- [x] [Review][Defer] Staged ZIPs under `bundle-staging/` are never garbage-collected — delete happens only on confirm success; every 422, abandoned upload, and FE "Remove" leaves the object forever (presign TTL expires the URL, not the object). Proper fix is a bucket lifecycle rule (infra/TF, outside story code); delete-on-422 would break legitimate retry (e.g. HYBRID_SHAPE_MISMATCH against the wrong skill). — deferred
- [x] [Review][Defer] Ingest's `putToS3` has the same missing `response.ok` check as the new bundle client [velara-web src/api/ingest.ts] — deferred, pre-existing (predates this story)
...
| 2026-07-08 | Code review (3-layer adversarial) → done. 16 patches applied: 🔴 bundle_key org-prefix validation (INVALID_BUNDLE_KEY — cross-org IDOR + arbitrary-delete), hybrid-only bundle enforcement, duplicate/non-canonical/overlong member-path rejection, corrupt-member read → 422, staged-ZIP HEAD size cap, narrow not-found except, threadpool extract+hash, dot-only filename fallback; FE putBundleToS3 res.ok check, bundle_key cleared on runtime switch, onUploadingChange submit guard; +21 BE / +4 FE tests. 1 decision resolved (bundle-run gap → defer to 11.2), 2 deferred (staging GC lifecycle rule; ingest putToS3 res.ok twin), 5 dismissed. Gates re-run green. |
